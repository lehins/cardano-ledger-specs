...
getSnapShotWithMaxSharing ::
     MonadResource m
  => [EpochBoundary.SnapShot C]
  -> Key EpochState
  -> SnapShotType
  -> ReaderT SqlBackend m (EpochBoundary.SnapShot C)
getSnapShotWithMaxSharing otherSnapShots epochStateId snapShotType = do
  let otherStakes =
        EpochBoundary.unStake . EpochBoundary._stake <$> otherSnapShots
  let otherPoolParams = EpochBoundary._poolParams <$> otherSnapShots
  let otherDelegations = EpochBoundary._delegations <$> otherSnapShots
  ...
  poolParams <-
    selectMap [SnapShotPoolSnapShotId ==. snapShotId] $ \SnapShotPool {..} -> do
      KeyHash keyHash <- getJust snapShotPoolKeyHashId
      let kh = Keys.coerceKeyRole keyHash
      pure
        ( interns kh otherPoolParams
        , internsVal kh snapShotPoolParams otherPoolParams)
  delegations <-
    selectMap [SnapShotDelegationSnapShotId ==. snapShotId] $ \SnapShotDelegation {..} -> do
      Credential credential <- getJust snapShotDelegationCredentialId
      KeyHash keyHash <- getJust snapShotDelegationKeyHash
...
getSnapShotsWithMaxSharing ::
     MonadResource m
  => Entity EpochState
  -> ReaderT SqlBackend m (EpochBoundary.SnapShots C)
getSnapShotsWithMaxSharing (Entity epochStateId EpochState {epochStateSnapShotsFee}) = do
  mark <- getSnapShotWithMaxSharing [] epochStateId SnapShotMark
  set <- getSnapShotWithMaxSharing [mark] epochStateId SnapShotSet
  go <- getSnapShotWithMaxSharing [mark, set] epochStateId SnapShotGo
  pure $
    EpochBoundary.SnapShots
      { _pstakeMark = mark
      , _pstakeSet = set
...
storeEpochState ::
  MonadUnliftIO m => FilePath -> Shelley.EpochState CurrentEra -> m ()
...
loadEpochStateEntity :: MonadUnliftIO m => FilePath -> m (Entity EpochState)
loadEpochStateEntity fp = runSqlite (T.pack fp) (getJustEntity esId)

loadSnapShotsNoSharing ::
     MonadUnliftIO m => FilePath -> Entity EpochState -> m (EpochBoundary.SnapShots C)
loadSnapShotsNoSharing fp = runSqlite (T.pack fp) . getSnapShotsNoSharing

loadSnapShotsWithSharing ::
     MonadUnliftIO m => FilePath -> Entity EpochState -> m (EpochBoundary.SnapShots C)
loadSnapShotsWithSharing fp = runSqlite (T.pack fp) . getSnapShotsWithSharing

loadSnapShotsWithMaxSharing ::
     MonadUnliftIO m => FilePath -> Entity EpochState -> m (EpochBoundary.SnapShots C)
loadSnapShotsWithMaxSharing fp = runSqlite (T.pack fp) . getSnapShotsWithMaxSharing
